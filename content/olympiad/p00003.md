+++
date = "2025-03-17"
draft = false
title = "Problem 00003"
code = "p00003"
aliases = ["/olympiad/pag03/"]
source = "Australian Math Olympiad, 1992"
problemTags = ["algebra", "inequalities"]
difficulty = "medium"
gradeLevel = ["9th"]
usekatex = true
origin = "The Trickonometry of Math Olympiad Inequalities"
+++

<div class="mp" id="p00003">
        <p><a class="mpl" href="/olympiad/p00003/">Problem 00003</a></p> 
        <p>Let \(n\) be a positive integer greater than \(1\). Show that:</p>
        <p class="mpc">
            \[
                \frac{1}{n}+\frac{1}{n+1}+\frac{1}{n+2}+\dots+\frac{1}{2n-1} \gt n \left(\sqrt[n]{2} - 1\right)
            \]
        </p>
        <details>
            <summary>Hint</summary>
            <p>Try adding \(n\) to both sides of the inequality and then applying the AM-GM inequality to the \(n\) terms on the left-hand side.</p>
        </details>
        <details>
            <summary>Solution</summary>
            <p>Adding \(n\) to both sides of the given inequality, we get:</p>
            <p class="mpc">
                \[
                    n + \frac{1}{n} + \frac{1}{n+1} + \dots + \frac{1}{2n-1} \gt n\sqrt[n]{2}
                \]
            </p>
            <p>We can rewrite the left-hand side by distributing the \(n\) ones into each fraction:</p>
            <p class="mpc">
                \[
                    \left(1+\frac{1}{n}\right) + \left(1+\frac{1}{n+1}\right) + \dots + \left(1+\frac{1}{2n-1}\right) \gt n\sqrt[n]{2}
                \]
            </p>
            <p>Simplifying the terms:</p>
            <p class="mpc">
                \[
                    \frac{n+1}{n} + \frac{n+2}{n+1} + \dots + \frac{2n}{2n-1} \gt n\sqrt[n]{2}
                \]
            </p>
            <p>By the AM-GM inequality for \(n\) positive terms:</p>
            <p class="mpc">
                \[
                    \frac{\frac{n+1}{n} + \frac{n+2}{n+1} + \dots + \frac{2n}{2n-1}}{n} \ge \sqrt[n]{\frac{n+1}{n} \cdot \frac{n+2}{n+1} \cdot \dots \cdot \frac{2n}{2n-1}}
                \]
            </p>
            <p>The product inside the radical is a telescoping product that simplifies to \(\frac{2n}{n} = 2\). Equality in AM-GM would require all the ratios to be equal. For \(n>1\), already \(\frac{n+1}{n}\neq\frac{n+2}{n+1}\), since equality would imply \((n+1)^2=n(n+2)\), or \(1=0\). Therefore the AM-GM inequality above is strict, and:</p>
            <p class="mpc">
                \[
                    \frac{LHS + n}{n} \gt \sqrt[n]{2} \implies LHS \gt n(\sqrt[n]{2} - 1)
                \]
            </p>
            <p>This completes the proof. Equality holds only for \(n=1\), which is excluded by the problem constraints.</p>
        </details>
        <details>
            <summary>Source</summary>
            <p>Australian Math Olympiad, 1992</p>
        </details>
    </div>
