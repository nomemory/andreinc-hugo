+++
date = "2025-03-17"
draft = false
title = "Problem 00024"
code = "p00024"
aliases = ["/olympiad/pcbs10/"]
source = "Iran, 1998"
problemTags = ["algebra", "inequalities"]
difficulty = "medium"
gradeLevel = ["9th"]
usekatex = true
origin = "The Trickonometry of Math Olympiad Inequalities"
+++

<div class="mp" id="p00024">
        <p><a class="mpl" href="/olympiad/p00024/">Problem 00024</a></p> 
        <p>Let \(x,y,z \ge 1\) such that \(\frac{1}{x}+\frac{1}{y}+\frac{1}{z}=2\). Prove the inequality:</p>
        <p class="mpc">
            \[
                \sqrt{x+y+z} \ge \sqrt{x-1} + \sqrt{y-1} + \sqrt{z-1}
            \]
        </p>
        <details>
            <summary>Hint 1</summary>
            <p>From the given condition, attempt to obtain a new expression involving \(x-1\), \(y-1\) and \(z-1\).</p>
        </details>
        <details>
            <summary>Solution</summary>
            <p>We begin by manipulating the given condition:</p>
            <p class="mpc">
                \[
                    \left(1-\frac{1}{x}\right)+\left(1-\frac{1}{y}\right) + \left(1-\frac{1}{z}\right) = 3-2 \Leftrightarrow
                \]
                \[
                    \frac{x-1}{x} + \frac{y-1}{y} + \frac{z-1}{z} = 1
                \]
            </p>
            <p>Now, apply the Cauchy-Schwarz inequality (CBS inequality) in the form:</p>
            <p class="mpc">
                \[
                    (x+y+z)(\frac{x-1}{x} + \frac{y-1}{y} + \frac{z-1}{z})
                    \overbrace{\ge}^{\text{CBS}} (\sqrt{x-1}+\sqrt{y-1}+\sqrt{z-1})^2
                \]
            </p>
            <p>Which, after taking the square root of both sides, simplifies to:</p>
            <p class="mpc">
                \[
                    \sqrt{x+y+z} \ge \sqrt{x-1}+\sqrt{y-1}+\sqrt{z-1}
                \]
            </p>
            <p>Equality holds if and only if \(x=y=z=\frac32\).</p>
        </details>
        <details>
            <summary>Source</summary>
            <p>Iran, 1998</p>
        </details>
    </div>
