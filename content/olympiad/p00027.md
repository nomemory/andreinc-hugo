+++
date = "2025-03-17"
draft = false
title = "Problem 00027"
code = "p00027"
aliases = ["/olympiad/pcbs13/"]
source = "Concursul Gazeta Matematica, Editia a IXa, 9th grade, Romania"
problemTags = ["algebra", "inequalities"]
difficulty = "medium"
gradeLevel = ["9th"]
usekatex = true
origin = "The Trickonometry of Math Olympiad Inequalities"
+++

<div class="mp" id="p00027">
    <p><a class="mpl" href="/olympiad/p00027/">Problem 00027</a></p> 
    <p>Let \(a\), \(b\), and \(c\) be positive real numbers satisfying \(a^2 + b^2 + c^2 \ge 4\). Prove that:</p>
<p class="mpc">
    \[
        \frac{a^3}{b+3c}+\frac{b^3}{c+3a}+\frac{c^3}{a+3b}\ge1
    \]
</p>
<details>
    <summary>Hint 1</summary>
    <p>How can you use the CBS inequality to make the condition \(a^2 + b^2 + c^2 \ge 4\) more relevant?</p>
</details>
<details>
    <summary>Hint 2</summary>
            <p>If \(a_1\), \(a_2\), \(a_3\) and \(b_1\), \(b_2\), \(b_3\) are positive real numbers, can you prove the following useful inequality?</p>
    <p class="mpc">
        \[
            a_1+a_2+a_3 \ge \frac{\left(\sqrt{a_1b_1}+\sqrt{a_2b_2}+\sqrt{a_3b_3}\right)^2}{b_1+b_2+b_3}
        \]
    </p>
</details>
<details>
    <summary>Solution</summary>
    <p>How can we use the fact that \(a^2+b^2+c^2\ge4\)?</p>
    <p>For \(a_1, a_2, a_3, b_1, b_2, b_3\) positive numbers, a direct consequence of the CBS inequality is:</p>
    <p class="mpc">
    \[
        (\sqrt{a_1}\cdot\sqrt{b_1}+\sqrt{a_2}\cdot\sqrt{b_2}+\sqrt{a_3}\cdot\sqrt{b_3})^2 \le (a_1+a_2+a_3)(b_1+b_2+b_3) \Leftrightarrow \\
        a_1 + a_2 + a_3 \ge \frac{(\sqrt{a_1b_1}+\sqrt{a_2b_2}+\sqrt{a_3b_3})^2}{b_1+b_2+b_3}
    \]
    </p>
    <p>The trick is to choose \(b_1, b_2, b_3\) in such a way that they simplify our inequality. Let:</p>
    <p class="mpc">
    \[
        \begin{cases}
            a_1 \rightarrow \frac{a^3}{b+3c} \\
            a_2 \rightarrow \frac{b^3}{c+3a} \\
            a_3 \rightarrow \frac{c^3}{a+3b} \\
            b_1 \rightarrow a(b+3c) \\
            b_2 \rightarrow b(c+3a) \\
            b_3 \rightarrow c(a+3b) \\
        \end{cases}
    \]
    </p>
    <p>By applying this substitution to the left-hand side of the inequality:</p>
    <p class="mpc">
        \[
            \sum_{\mathrm{cyc}} \frac{a^3}{b+3c} \ge \frac{\left[\sqrt{\frac{a^3}{b+3c}\cdot a(b+3c)}+\sqrt{\frac{b^3}{c+3a}\cdot b(c+3a)}+\sqrt{\frac{c^3}{a+3b}\cdot c(a+3b)}\right]^2}{a(b+3c)+b(c+3a)+c(a+3b)} =
        \]
    </p>
    <p>After simplifying:</p>
    <p class="mpc">
        \[
            = \frac{(a^2+b^2+c^2)^2}{4(ab+bc+ca)} = \frac{a^2+b^2+c^2}{4}\cdot \frac{a^2+b^2+c^2}{ab+bc+ca} \ge 1
        \]
    </p>
    <p>We've already proven that \(a^2+b^2+c^2 \ge ab+bc+ca\), and we know that \(a^2+b^2+c^2 \ge 4\) so the inequality is true.</p>
    <p>Equality holds when \(a=b=c=\frac{2}{\sqrt{3}}\).</p>
</details>
<details>
    <summary>Source</summary>
    <p>Concursul Gazeta Matematica, Editia a IXa, 9th grade, Romania</p>
</details>
</div>
