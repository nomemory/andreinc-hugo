+++
date = "2025-03-17"
draft = false
title = "Problem 00033"
code = "p00033"
aliases = ["/olympiad/pgtm02/"]
source = ""
problemTags = ["algebra", "inequalities"]
difficulty = "easy"
gradeLevel = ["9th"]
usekatex = true
origin = "The Trickonometry of Math Olympiad Inequalities"
+++

<div class="mp" id="p00033">
        <p><a class="mpl" href="/olympiad/p00033/">Problem 00033</a></p> 
        <p>Let \(x,y,z\) be positive real numbers such that \((1+x)(1+y)(1+z)=8\). Prove that \(xyz \le 1\).</p>
        <details>
            <summary>Hint 1</summary>
            <p>The terms in the product are already grouped for us.</p>
        </details>
        <details>
            <summary>Solution</summary>
            <p>We apply the Arithmetic Mean-Geometric Mean (AM-GM) inequality to each term in the product:</p>
            <p class="mpc">
                \[
                    (1+x) \ge 2\sqrt{x}, (1+y) \ge 2\sqrt{y} \text{ and } (1+z) \ge 2\sqrt{z}
                \]
            </p>
            <p>Thus, we have:</p>
            <p class="mpc">
                \[
                    8 = \underbrace{(1+x)}_{\ge 2\sqrt{x}}\underbrace{(1+y)}_{\ge 2\sqrt{y}}\underbrace{(1+z)}_{\ge2\sqrt{z}} \ge 8\sqrt{xyz} 
                \]
            </p>
            <p>Since we are given \((1+x)(1+y)(1+z)=8\), it follows that:</p>
            <p class="mpc">
                \[
                    8 \ge 8 \sqrt{xyz} \Leftrightarrow 1 \ge xyz
                \]
            </p>
            <p>Hence, the inequality is proven. Equality holds if and only if \(x=y=z=1\).</p>
        </details>
    </div>
