+++
date = "2025-03-17"
draft = false
title = "Problem 00055"
code = "p00055"
aliases = ["/olympiad/pgtm23/"]
source = "Andrei N. Ciobanu"
problemTags = ["algebra", "inequalities"]
difficulty = "medium"
gradeLevel = ["9th"]
usekatex = true
origin = "The Trickonometry of Math Olympiad Inequalities"
+++

<div class="mp" id="p00055">
    <p><a class="mpl" href="/olympiad/p00055/">Problem 00055</a></p> 
    <p>Let \(a,b,c \in (0,\infty)\) such that \(ab+bc+ca=abc\). Prove that:</p>
    <p class="mpc">
    \[
        3\sqrt{abc} \gt 2\sqrt{2}(\sqrt{a}+\sqrt{b}+\sqrt{c})
    \]
    </p>
<details> 
    <summary>Hint 1</summary>
    <p>Can you show that:</p>
    <p>
        \[
            \frac{1}{a}+\frac{1}{b}+\frac{1}{c}=1
        \]
    </p>
</details>
<details> 
    <summary>Hint 2</summary>
    <p>Consider applying the AM-GM inequality to the following groupings:</p>
    <p class="mpc">
    \[
        \begin{cases}
            \frac{1}{a}+\frac{1}{a}+\frac{1}{b} = \frac{2}{a}+\frac{1}{b} \ge 2\sqrt{\frac{2}{ab}} \\
            \frac{1}{b}+\frac{1}{b}+\frac{1}{c} = \frac{2}{b}+\frac{1}{c} \ge 2\sqrt{\frac{2}{bc}} \\
            \frac{1}{c}+\frac{1}{c}+\frac{1}{a} = \frac{2}{c}+\frac{1}{a} \ge 2\sqrt{\frac{2}{ca}} 
        \end{cases}
    \]
    </p>
</details>
<details> 
    <summary>Solution</summary>
    <p>Start by dividing both sides of the equation \(abc=bc+ac+ab\) by \(abc\), which yields:</p>
    <p class="mpc">
        \[
            \frac{1}{a}+\frac{1}{b}+\frac{1}{c}=1
        \]
    </p>
    <p>Next, apply the AM-GM inequality to the following groupings of terms:</p>
    <p class="mpc">
        \[
            \frac{1}{a}+\frac{1}{a}+\frac{1}{b} = \frac{2}{a}+\frac{1}{b} \ge 2\sqrt{\frac{2}{ab}} \\
            \frac{1}{b}+\frac{1}{b}+\frac{1}{c} = \frac{2}{b}+\frac{1}{c} \ge 2\sqrt{\frac{2}{bc}} \\
            \frac{1}{c}+\frac{1}{c}+\frac{1}{a} = \frac{2}{c}+\frac{1}{a} \ge 2\sqrt{\frac{2}{ca}} 
        \]
    </p>
    <p>Summing these inequalities results in:</p>
    <p class="mpc">
    \[
     3(\frac{1}{a}+\frac{1}{b}+\frac{1}{c}) \ge 2\sqrt{2}(\frac{1}{\sqrt{ab}} + \frac{1}{\sqrt{bc}}+\frac{1}{\sqrt{ca}})
     \]
    </p>
    <p>Equality in the three AM-GM inequalities would require \(a=2b\), \(b=2c\), and \(c=2a\), which is impossible for positive variables. Hence the preceding inequality is strict, and using \(\frac1a+\frac1b+\frac1c=1\) gives:</p>
    <p class="mpc">
     \[
     3 \gt 2\sqrt{2}(\frac{1}{\sqrt{ab}} + \frac{1}{\sqrt{bc}}+\frac{1}{\sqrt{ca}}) \Leftrightarrow \\
     3\sqrt{abc} \gt 2\sqrt{2}(\sqrt{c}+\sqrt{a}+\sqrt{b})
     \]
    </p>
    <p>Thus, the inequality holds.</p>
</details>
<details> 
    <summary>Source</summary>
    <p>Andrei N. Ciobanu</p>
</details>
</div>
