+++
date = "2025-03-17"
draft = false
title = "Problem 00064"
code = "p00064"
aliases = ["/olympiad/pgtm30/"]
source = "IMO 2000"
problemTags = ["algebra", "inequalities"]
difficulty = "medium"
gradeLevel = ["9th"]
usekatex = true
origin = "The Trickonometry of Math Olympiad Inequalities"
+++

<div class="mp" id="p00064">
        <p><a class="mpl" href="/olympiad/p00064/">Problem 00064</a></p> 
        <p>Let \(a,b,c\) positive real numbers such that \(abc=1\). Prove that:</p>
        <p class="mpc">
            \[
                \left(a-1+\frac{1}{b}\right)\left(b-1+\frac{1}{c}\right)\left(c-1+\frac{1}{a}\right) \leq 1
            \]
        </p>
        <details>
            <summary>Hint 1</summary>
            <p>If \(abc=1\) it's always a good idea to perform the following substition (homogenisation):</p>
            <p class="mpc">
                \[
                    a=\frac{x}{y}, \quad b=\frac{y}{z}, \quad c=\frac{z}{x}
                \]
            </p>
        </details>
        <details>
            <summary>Solution</summary>
            <p>Since \(abc=1\) we can perform the following substition:</p>
            <p class="mpc">
                \[
                a=\frac{x}{y}, \quad b=\frac{y}{z}, \quad c=\frac{z}{x}
                \]
            </p>
            <p>The given inequality transforms and simplifies into/to:</p>
            <p class="mpc">
                \[
                    \left(\frac{x}{y}-1+\frac{z}{y}\right)\left(\frac{y}{z}-1+\frac{x}{z}\right)\left(\frac{z}{x}-1+\frac{y}{x}\right) \leq 1 \Leftrightarrow \\
                    (x-y+z)(y-z+x)(z-x+y) \leq xyz
                \]
            </p>
            <p>Write \(A=x-y+z\), \(B=y-z+x\), and \(C=z-x+y\). If one of these numbers is non-positive, the other two are positive. For example, \(A\leq0\) implies \(y\geq x+z\), so \(B\geq2x\gt0\) and \(C\geq2z\gt0\). Thus \(ABC\leq0\lt xyz\), and the desired inequality is immediate. The other cases are analogous.</p>
            <p>It remains to consider \(A,B,C\gt0\). We can now use the AM-GM inequality:</p>
            <p class="mpc">
                \[
                    (x-y+z)+(y-z+x) = 2x \geq 2\sqrt{(x-y+z)(y-z+x)} \\
                    (y-z+x)+(z-x+y) = 2y \geq 2\sqrt{(y-z+x)(z-x+y)} \\
                    (z-x+y)+(x-y+z) = 2z \geq 2\sqrt{(z-x+y)(x-y+z)} \\
                \]
            </p>
            <p>Multiplying all three inequalities:</p>
            <p class="mpc">
                \[
                    8xyz \geq 8(x-y+z)(y-z+x)(z-x+y) \Leftrightarrow \\
                    xyz \geq (x-y+z)(y-z+x)(z-x+y)
                \]
            </p>
            <p>Thus, the inequality is proven. Equality holds if and only if \(x=y=z\), which is equivalent to \(a=b=c=1\).</p>
        </details>
        <details>
            <summary>Source</summary>
            <p>IMO 2000</p>
        </details>
    </div>
