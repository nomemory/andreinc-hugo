+++
date = "2025-03-17"
draft = false
title = "Problem 00092"
code = "p00092"
aliases = ["/olympiad/pmch01/"]
source = "Romanian Math Olympiad, 10th grade, 2006"
problemTags = ["algebra", "inequalities"]
difficulty = "medium"
gradeLevel = ["10th"]
usekatex = true
origin = "The Trickonometry of Math Olympiad Inequalities"
+++

<div class="mp" id="p00092">
        <p><a class="mpl" href="/olympiad/p00092/">Problem 00092</a></p> 
        <p>Let \(a,b,c \in (0,1)\) and \(x,y,z \in (0, \infty)\) such that:</p>
        <p class="mpc">
            \[
                a^x=bc, \quad b^y=ca, \quad c^z = ab
            \]
        </p>
        <p>Prove the inequality:</p>
        <p class="mpc">
            \[
                \frac{1}{2+x}+\frac{1}{2+y} + \frac{1}{2+z} \leq \frac{3}{4}
            \]
        </p>
        <details>   
            <summary>Hint 1</summary>
            <p>Express \(x,y,z\) in terms of logarithms:</p>
            <p class="mpc">
                \[
                    x = \log_a bc, \quad y = \log_b ca, \quad z = \log_c ab \\
                \]
            </p>
        </details>
        <details>
            <summary>Hint 2</summary>
            <p>Since \(a,b,c\in(0,1)\), introduce the positive quantities \(\alpha=-\ln a\), \(\beta=-\ln b\), and \(\gamma=-\ln c\):</p>
            <p class="mpc">
                \[
                    x = \frac{\beta+\gamma}{\alpha}, \quad y = \frac{\gamma+\alpha}{\beta}, \quad z = \frac{\alpha+\beta}{\gamma}
                \]
            </p>
        </details>
        <details>
            <summary>Solution</summary>
            <p>First, we take natural logarithms in the three given equations:</p>
            <p class="mpc">
                \[
                     x = \log_a bc =  \frac{\ln b + \ln c}{\ln a} \\
                     y = \log_b ca =  \frac{\ln c + \ln a}{\ln b} \\
                     z = \log_c ab =  \frac{\ln a + \ln b}{\ln c} \\
                \]
            </p>
            <p>To simplify notation, introduce:</p> 
            <p class="mpc">
            \[
                \alpha=-\ln a, \quad \beta =-\ln b, \quad \gamma =-\ln c
            \]
            </p>
            <p>These three quantities are positive, and we can rewrite \(x,y,z\) as:</p>
            <p class="mpc">
                \[
                    x = \frac{\beta+\gamma}{\alpha}, \quad y = \frac{\gamma+\alpha}{\beta}, \quad z = \frac{\alpha+\beta}{\gamma}
                \]
            </p>
            <p>Thus, our inequality transforms into:</p>
            <p class="mpc">
                \[
                    \frac{1}{2+\frac{\beta+\gamma}{\alpha}} + \frac{1}{2+\frac{\gamma+\alpha}{\beta}} + \frac{1}{2+\frac{\alpha+\beta}{\gamma}} \le \frac{3}{4}
                \]
            </p>
            <p>Changing the signs and adding \(3\) to each side:</p>
            <p class="mpc">
                \[
                    \left(1-\frac{1}{2+\frac{\beta+\gamma}{\alpha}}\right)+\left(1-\frac{1}{2+\frac{\gamma+\alpha}{\beta}}\right)+\left(1-\frac{1}{2+\frac{\alpha+\beta}{\gamma}}\right) \geq \frac{9}{4}
                \]
            </p>
            <p>Define \(S=\alpha+\beta+\gamma\):</p>
            <p class="mpc">
                \[
                    \left( 1-\frac{\alpha}{S+\alpha} \right)+\left( 1-\frac{\beta}{S+\beta} \right)+\left( 1-\frac{\gamma}{S+\gamma} \right) \ge \frac{9}{4}
                \]
            </p>
            <p>Applying Titu's Lemma:</p>
            <p class="mpc">
                \[
                    S\left( \frac{1}{S+\alpha} + \frac{1}{S+\beta} + \frac{1}{S+\gamma} \right) \ge S*\frac{(1+1+1)^2}{3*S + (\alpha+\beta+\gamma)} = \frac{9S}{4S} = \frac{9}{4}
                \]
            </p>
            <p>This proves the required inequality.</p> 
            <p>Equality in Titu's lemma occurs if and only if \(\alpha=\beta=\gamma\). Thus equality in the original inequality holds if and only if \(a=b=c\), in which case the three equations give \(x=y=z=2\).</p>
        </details>
        <details>
            <summary>Source</summary>
            <p>Romanian Math Olympiad, 10th grade, 2006</p>
        </details>
    </div>
