+++
date = "2025-03-17"
draft = false
title = "Problem 00110"
code = "p00110"
aliases = ["/olympiad/pnbt05/"]
source = ""
problemTags = ["algebra", "inequalities"]
difficulty = "medium"
gradeLevel = ["9th"]
usekatex = true
origin = "The Trickonometry of Math Olympiad Inequalities"
+++

<div class="mp" id="p00110">
        <p><a class="mpl" href="/olympiad/p00110/">Problem 00110</a></p> 
        <p>Let \(a,b,c\) be positive real numbers such that \(a+b+c=1\). Prove the inequality:</p>
        <p class="mpc">
            \[
                \frac{1+a}{1-a}+\frac{1+b}{1-b}+\frac{1+c}{1-c} \ge 6
            \]
        </p>
        <details>
            <summary>Hint 1</summary>
            <p>Note that if \(a + b + c = 1\), then \(1 - a = b + c\), and similarly for \(b\) and \(c\).</p>
        </details>
        <details>
            <summary>Solution</summary>
            <p>Using the given condition \(a + b + c = 1\), we rewrite:</p>
            <p class="mpc">
                \[
                    1 - a = b + c, \quad 1 - b = a + c, \quad 1 - c = a + b.
                \]
            </p>
            <p>Similarly, we express the numerators:</p>
            <p class="mpc">
                \[
                    1 + a = 2a + b + c, \quad 1 + b = 2b + a + c, \quad 1 + c = 2c + a + b.
                \]
            </p>
            <p>Substituting these into the given expression:</p>
            <p class="mpc">
                \[
                    \frac{1 + a}{1 - a} + \frac{1 + b}{1 - b} + \frac{1 + c}{1 - c} =
                    \frac{2a + b + c}{b + c} + \frac{2b + a + c}{a + c} + \frac{2c + a + b}{a + b}.
                \]
            </p>
            <p>Simplifying each fraction:</p>
            <p class="mpc">
                \[
                    \frac{2a + b + c}{b + c} = 1 + \frac{2a}{b + c}, \quad
                    \frac{2b + a + c}{a + c} = 1 + \frac{2b}{a + c}, \quad
                    \frac{2c + a + b}{a + b} = 1 + \frac{2c}{a + b}.
                \]
            </p>
            <p>Thus, the inequality reduces to:</p>
            <p class="mpc">
                \[
                    3+2\left(\frac{a}{b+c}+\frac{b}{a+c}+\frac{c}{a+b}\right) \ge 6.
                \]
            </p>
            <p>By Nesbitt's inequality:</p>
            <p class="mpc">
                \[
                    \frac{a}{b + c} + \frac{b}{a + c} + \frac{c}{a + b} \ge \frac{3}{2},
                \]
            </p>
            <p>so the original expression is at least \(3+2\cdot\frac32=6\). Equality holds when \(a=b=c=\frac13\).</p>
        </details>
    </div>
