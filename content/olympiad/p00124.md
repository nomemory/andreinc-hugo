+++
date = "2025-03-17"
draft = false
title = "Problem 00124"
code = "p00124"
aliases = ["/olympiad/prns01/"]
source = ""
problemTags = ["algebra", "inequalities"]
difficulty = "medium"
gradeLevel = ["9th"]
usekatex = true
origin = "The Trickonometry of Math Olympiad Inequalities"
+++

<div class="mp" id="p00124">
        <p><a class="mpl" href="/olympiad/p00124/">Problem 00124</a></p> 
        <p>Let \(x, y, z, a, b\) be positive real numbers. Prove the following inequality:</p>
        <p class="mpc">
            \[
                \frac{x}{ay+bz}+\frac{y}{az+bx}+\frac{z}{ax+by} \ge \frac{3}{a+b}
            \]
        </p>
        <details>
            <summary>Hint 1</summary>
            <p>Use \((x+y+z)^2\ge 3(xy+yz+zx)\) after applying Cauchy-Schwarz.</p>
            <p>For positive real numbers \(a_1, a_2, a_3, b_1, b_2, b_3\), a direct consequence of the Cauchy-Schwarz inequality is:</p>
            <p class="mpc">
                \[
                    (\sqrt{a_1} \cdot \sqrt{b_1} + \sqrt{a_2} \cdot \sqrt{b_2} + \sqrt{a_3} \cdot \sqrt{b_3})^2 \le (a_1 + a_2 + a_3)(b_1 + b_2 + b_3) \\
                    \Rightarrow a_1 + a_2 + a_3 \ge \frac{(\sqrt{a_1b_1} + \sqrt{a_2b_2} + \sqrt{a_3b_3})^2}{b_1 + b_2 + b_3}.
                \]
            </p>
        </details>
        <details>
            <summary>Solution</summary>
            <p>As suggested, we apply the Cauchy-Schwarz inequality in the following manner:</p>
            <p class="mpc">
                \[
                    \frac{x}{ay + bz} + \frac{y}{az + bx} + \frac{z}{ax + by} \ge \frac{(x + y + z)^2}{x(ay + bz) + y(az + bx) + z(ax + by)}.
                \]
            </p>
            <p>Now, simplify the denominator:</p>
            <p class="mpc">
                \[
                    \frac{x}{ay + bz} + \frac{y}{az + bx} + \frac{z}{ax + by} \ge \frac{(x + y + z)^2}{a(xy + yz + zx) + b(xy + yz + zx)}.
                \]
            </p>
            <p>Factoring out \(xy + yz + zx\) from the denominator, we obtain:</p>
            <p class="mpc">
                \[
                    \frac{x}{ay + bz} + \frac{y}{az + bx} + \frac{z}{ax + by} \ge \frac{(x + y + z)^2}{(a + b)(xy + yz + zx)}.
                \]
            </p>       
            <p>We know that \((x + y + z)^2 \ge 3(xy + yz + zx)\), so substituting this inequality into the previous expression gives:</p>
            <p class="mpc">
                \[
                    \frac{x}{ay + bz} + \frac{y}{az + bx} + \frac{z}{ax + by} \ge \frac{3(xy + yz + zx)}{(a + b)(xy + yz + zx)}.
                \]
            </p>
            <p>Simplifying the right-hand side, we get:</p>
            <p class="mpc">
                \[
                    \frac{x}{ay + bz} + \frac{y}{az + bx} + \frac{z}{ax + by} \ge \frac{3}{a + b}.
                \]
            </p>
            <p>Equality holds when \(x=y=z\).</p>
        </details>
    </div>
