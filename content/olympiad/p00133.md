+++
date = "2025-03-17"
draft = false
title = "Problem 00133"
code = "p00133"
aliases = ["/olympiad/ptt04/"]
source = ""
problemTags = ["algebra", "inequalities"]
difficulty = "medium"
gradeLevel = ["9th"]
usekatex = true
origin = "The Trickonometry of Math Olympiad Inequalities"
+++

<div class="mp" id="p00133">
    <p><a class="mpl" href="/olympiad/p00133/">Problem 00133</a></p> 
    <p>Let \(a,b,c\) be positive real numbers. Prove:</p>
    <p class="mpc">
        \[
            \frac{a}{b+c}+\frac{b}{a+c}+\frac{c}{a+b}\ge\frac{3}{2}
        \]
    </p>
    <details>
        <summary>Hint 1</summary>
        <p>Rewrite each term in a form that suggests an application of Titu's Lemma:</p>
        <p class="mpc">
        \[
            \frac{a}{b+c}=\frac{a^2}{a(b+c)}
        \]
        </p>
    </details>
    <details>
        <summary>Solution</summary>
        <p>To align with Titu's Lemma, we rewrite each term on the left-hand side:</p>  
        <p class="mpc">
            \[
                \frac{a}{b+c}+\frac{b}{a+c}+\frac{c}{a+b}=\frac{a^2}{a(b+c)}+\frac{b^2}{b(a+c)}+\frac{c^2}{c(a+b)}
            \]
        </p>
        <p>Applying Titu’s Lemma, we obtain the following lower bound:</p>
        <p class="mpc">
            \[
                \frac{a^2}{a(b+c)}+\frac{b^2}{b(a+c)}+\frac{c^2}{c(a+b)} \ge \frac{(a+b+c)^2}{2(ab+bc+ca)} = \\ 
                \frac{a^2+b^2+c^2+2(ab+bc+ca)}{2(ab+bc+ca)} = \\ 
                = 1 + \frac{a^2+b^2+c^2}{2(ab+bc+ca)} \ge \frac{3}{2}
            \]
        </p>
        <p>We conclude that:</p>   
        <p class="mpc">
            \[
                \frac{a^2+b^2+c^2}{2(ab+bc+ca)}\ge\frac{1}{2} \Leftrightarrow a^2 + b^2 + c^2 \ge ab + bc + ca
            \]
        </p>
         <p>which is a known inequality that completes the proof. Equality holds exactly when \(a=b=c\).</p>
    </details>
</div>
