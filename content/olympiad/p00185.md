...
<p>Let \(a,b \in (0, \infty) \setminus \{1\}\) with \(ab\ne1\), and let \(x,y,n\in\mathbb R\) with \(n\ne0\). Given the following equations involving a common exponent \(n\):</p>
<p>
    \[
...
<details> 
    ...
    <p>Because \(ab\ne1\) and \(n\ne0\), the common value \((ab)^n\) is not \(1\); hence \(x\ne0\) and \(y\ne0\), so all reciprocals below are defined. Starting with the first equation, we use logarithms to isolate \(x\) and \(y\):</p>
    <p>
        \[
...
<details>
    ...
    <p>After giving this more thought, a simpler solution without logarithms exists. Can you find it?</p>
</details>
</div>
