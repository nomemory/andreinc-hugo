...
{{< img src="/images/2022-05-01-4-integers-are-enough-to-write-a-snake-game/snake.gif" >}}
...
* We will store the game map in a `uint32_t` where `1`s will form the reptile's body. The map will contain `4x8` positions. Enough to have fun!
...
* We will squeeze a few more 5-bit values into an `uint32_t` to keep the positions of the `head`, the `tail`, the `apple`, and the (current) `length`. Any input from the keyboard will also be kept here (2 bits will be enough).
...
{{< img src="/images/2022-05-01-4-integers-are-enough-to-write-a-snake-game/map.drawio.png" width="400" >}}
...
{{< img src="/images/2022-05-01-4-integers-are-enough-to-write-a-snake-game/vars.drawio.png" width="400" >}}
...
* the remaining bits are not used. Normally we could have kept the `uint8_t` counter here, but for simplicity, I chose to use a separate variable.
...
For more information describing the technique behind these macros, please read the following article: [*Working with bits and bitfields*](https://www.coranac.com/documents/working-with-bits-and-bitfields/).
...
{{< img src="/images/2022-05-01-4-integers-are-enough-to-write-a-snake-game/shape.drawio.png" width="400" >}}
...
{{< img src="/images/2022-05-01-4-integers-are-enough-to-write-a-snake-game/shapequeue.drawio.png" width="400" >}}
...
* `check_l()` - we check if the X coordinate of the snake (the modulo `%8` of `s_hpos`) is larger than the one from the previous position.
* `check_r()` - we check if the X coordinate of the snake (the modulo `%8` of `s_hpos`) is smaller than the one from the previous position. 
* `check_u()` and `check_d()` work similarly; they check if incrementing/decrementing `s_hpos` causes it to overflow/underflow. If it does, it means we have exited the bounds of the grid. Here, integer overflows are used as features. 
...
It was a fun exercise. The full code can be found [here](https://github.com/nomemory/integers-snake/blob/main/snake.c); it's around 100 lines and just 4 integers. 
